...
def calc_magnetization(states_):
    return np.sum(np.sum(np.sum(np.sum(np.tensordot(states_,local_axis,[[4],[0]]),4),3),2),1)
...
def check_eta(Nx, Ny, Nz,
              ...
              verbose = False, mode="real", radius_of_summation=10):
    global eta
    global nearest_neighbors
    try:
    ...
    remove_z_replicas = 0
    if mode == "open":
        remove_z_replicas = radius_of_summation
    
    for i in arange(-radius_of_summation, radius_of_summation+1):
        for j in arange(-radius_of_summation, radius_of_summation+1):
            for k in arange(-(radius_of_summation-remove_z_replicas), (radius_of_summation-remove_z_replicas)+1):
    ...
    try:
        if mode == "open":
            eta = (J1*load("interaction-matrices/eta_J1_"+str(Nx)+str(Ny)+str(Nz)+"_open.npy") +
                   J2*load("interaction-matrices/eta_J2_"+str(Nx)+str(Ny)+str(Nz)+"_open.npy") +
                   J3a*load("interaction-matrices/eta_J3a_"+str(Nx)+str(Ny)+str(Nz)+"_open.npy") +
                   J3b*load("interaction-matrices/eta_J3b_"+str(Nx)+str(Ny)+str(Nz)+"_open.npy"))
            nearest_neighbors = where(load("interaction-matrices/eta_J1_"+str(Nx)+str(Ny)+str(Nz)+"_open.npy") != 0, 1, 0)
        else:
            if radius_of_summation == 0:
                eta = (J1*load("interaction-matrices/eta_J1_"+str(Nx)+str(Ny)+str(Nz)+"_r"+str(radius_of_summation)+".npy") +
                ...
                eta = (J1*load("interaction-matrices/eta_J1_"+str(Nx)+str(Ny)+str(Nz)+".npy") +
        ...
        if mode == "open":
            ...
            if radius_of_summation == 0:
                ...
                save("interaction-matrices/eta_J1_"+str(Nx)+str(Ny)+str(Nz)+".npy", eta_J1)
                save("interaction-matrices/eta_J2_"+str(Nx)+str(Ny)+str(Nz)+".npy", eta_J2)
                save("interaction-matrices/eta_J3a_"+str(Nx)+str(Ny)+str(Nz)+".npy", eta_J3a)
                save("interaction-matrices/eta_J3b_"+str(Nx)+str(Ny)+str(Nz)+".npy", eta_J3b)
        eta = J1*eta_J1 + J2*eta_J2 + J3a*eta_J3a + J3b*eta_J3b
        nearest_neighbors = where(eta_J1 != 0, 1, 0)
    ...
    #         eta += 2.0*D*(ann**3)*load("1interaction-matrices/eta_D_"+str(Nx)+str(Ny)+str(Nz)+"_r"+str(radius_of_summation)+".npy")
            eta_D = load("interaction-matrices/eta_D_"+str(Nx)+str(Ny)+str(Nz)+"_r"+str(radius_of_summation)+".npy")
            eta_R = load("interaction-matrices/eta_R_"+str(Nx)+str(Ny)+str(Nz)+"_r"+str(radius_of_summation)+".npy")
        elif mode == "open":
    ...
    #         eta += 2.0*D*(ann**3)*load("1interaction-matrices/eta_D_"+str(Nx)+str(Ny)+str(Nz)+"_r"+str(radius_of_summation)+"_real.npy")
            eta_D = load("interaction-matrices/eta_D_"+str(Nx)+str(Ny)+str(Nz)+"_r"+str(radius_of_summation)+"_real.npy")
            eta_R = load("interaction-matrices/eta_R_"+str(Nx)+str(Ny)+str(Nz)+"_r"+str(radius_of_summation)+"_real.npy")
    ...
    if mode == "Ewald":
        save("interaction-matrices/eta_D_"+str(Nx)+str(Ny)+str(Nz)+"_r"+str(radius_of_summation)+".npy", eta_D)
        ...
        save("interaction-matrices/eta_D_"+str(Nx)+str(Ny)+str(Nz)+"_r"+str(radius_of_summation)+"_real.npy", eta_D)
        save("interaction-matrices/eta_R_"+str(Nx)+str(Ny)+str(Nz)+"_r"+str(radius_of_summation)+"_real.npy", eta_R)
        eta +=    D*ann**3*eta_D
        eta +=    R*ann**3*eta_R
    # if gpu_enabled == True:
    try:
...
def spin_ice_kmc(initial_state, N_steps = 1000, Temp = np.ones(1000), Nx=8,Ny=8,Nz=8, flip_mode="two-rate", use_gpu=True, verbose = False,
              ...
              # D = 0.0; J1 = -5.7; J2 = 0.0; J3a = 0.0; J3b = 0.0                 # + # H_NN Hallen 2022
              # D = 1.3224; J1 = 0.0; J2 = 0.0; J3a = 0.0; J3b = 0.0               # + # Test
                hfields = 0, eta_mode = "real"):
    check_eta(Nx, Ny, Nz,
              D = D, J1 = J1, J2 = J2, J3a = J3a,     J3b = J3b, R = R,
              verbose = verbose, mode = eta_mode)
    time0 = time.time()
